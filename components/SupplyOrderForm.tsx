...
import { createSupplyOrder, type FormState } from "@/app/actions";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
          <div className="w-full max-w-xl rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <h2 className="text-lg font-semibold text-slate-950">Create Supply Order</h2>
              <button
                type="button"
            ...
            <form action={formAction} className="space-y-4 px-5 py-5">
              ...
              <Field label="Material" error={state.errors?.material?.[0]}>
                <select
                  name="material"
                  required
              ...
              <Field label="Quantity in kg" error={state.errors?.quantityKg?.[0]}>
                <input
                  name="quantityKg"
                  type="number"
                  min="0.01"
              ...
              <Field label="Supplier name" error={state.errors?.supplierName?.[0]}>
                <input
                  name="supplierName"
                  className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                />
              ...
              <Field label="Tracking code" error={state.errors?.trackingCode?.[0]}>
                <input
                  name="trackingCode"
                  required
                  className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
              ...
              <Field label="ETA" error={state.errors?.eta?.[0]}>
                <input
                  name="eta"
                  type="datetime-local"
                  required
              ...
              <div className="flex justify-end gap-3 border-t border-slate-200 pt-4">
                ...
                >
                  ...
                  {pending ? "Creating..." : "Create Supply"}
                </button>
              </div>
            </form>
          </div>
